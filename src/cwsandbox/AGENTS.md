...
This directory contains the cwsandbox SDK implementation.
...
5. Add tests in `tests/unit/cwsandbox/test_sandbox.py`
...
- **Transient-retry mechanism** (`_retry.py`): the idempotent sandbox reads (status polling, `get_status`, `list`, `read_file`) share `retry_transient_async`. Transient-vs-fatal is classified in one place by `classify`/`is_retryable`, dispatching on exception class against `_RETRYABLE_EXCEPTIONS` (`SandboxUnavailableError`, `SandboxRequestTimeoutError`, `SandboxResourceExhaustedError`); `SandboxNotFoundError` short-circuits to fatal. To add a retryable condition, create a subclass of the appropriate umbrella exception and add it to `_RETRYABLE_EXCEPTIONS`; do not widen to a parent class, or callers catching that parent see unexpected retries. Mutating/streaming RPCs (`start`, `exec`) are never auto-retried — they only translate transport errors to typed exceptions. A call site may pass a narrower `should_retry` predicate when a normally-retryable code means something else there (e.g. `read_file` excludes `RESOURCE_EXHAUSTED`).
