...
# Server-enforced per-file cap for unary file write/read RPCs. The server
# may report a different value via CWSANDBOX_FILE_TOO_LARGE metadata; this
...
MAX_AUTO_FALLBACK_BYTES: int = 256 * 1024 * 1024

# Chunk size used by write_file_streaming when given a bytes object.
STREAMING_WRITE_CHUNK_SIZE: int = 1024 * 1024

# Max bytes of stderr buffered during read_file_streaming. The streaming
# read path captures only the command's exit status detail; full stderr
# would defeat the streaming goal.
STREAMING_READ_STDERR_CAP_BYTES: int = 16 * 1024
