...
- **Exec API**: `exec()` returns a `Process` object. Call `.result()` to block for the final `ProcessResult`. By default, output is silent (access via `result.stdout`). Iterate `process.stdout` before `.result()` for real-time streaming, or for convenience, use `quiet=False` to auto-print without manual iteration.
