...
| File | Coverage |
|------|----------|
| `test_sandbox.py` | Sandbox lifecycle, file ops, exec, working_dir, scratch volume, runtime_class, TLS passthrough, HTTPS share-token. Uses `require_auth`; TLS and share-token skip when no runner advertises them. |
