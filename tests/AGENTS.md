...
```
tests/
├── unit/aviato/        # Mock-based tests, no network calls (284 tests)
│   ├── conftest.py     # Auth env var clearing (autouse=True)
│   └── test_*.py       # 10 test files (one per source module + utilities)
└── integration/aviato/ # Real backend operations (31 tests)
...
### Unit Test Fixtures (`tests/unit/aviato/conftest.py`)

| Fixture | Scope | Autouse | Description |
|---------|-------|---------|-------------|
| `clean_auth_env` | function | Yes | Clears all auth env vars before each test |
| `mock_aviato_api_key` | function | No | Sets `AVIATO_API_KEY` to `"test-api-key"`, returns value |
| `mock_aviato_base_url` | function | No | Sets `AVIATO_BASE_URL` to `"http://test-api.example.com"`, returns value |
...
### Integration Test Fixtures (`tests/integration/aviato/conftest.py`)
...
1. `AVIATO_API_KEY` environment variable (takes priority)
...
### Unit Test Files (`tests/unit/aviato/`)
...
| `test_cli_logs.py` | CLI logs command, follow flag, options, not-found error |
| `test_cli_shell.py` | CLI shell command, happy-path session, Windows guard, not-found error |
