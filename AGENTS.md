...
- `start()`: Send create request, return `OperationRef[None]`. Call `.result()` to block until backend accepts. Freezes one create `request_id` for idempotent retries. If an accepted share-token create omitted its token, calling `start()` again replays the same request to recover it without creating another sandbox.
...
- `endpoint_share_token`: Create-only `EndpointShareToken` for `auth=SHARE_TOKEN` HTTPS URLs. Present on the create handle after Create / FromTemplate. The client replays an accepted create up to three times when the first response omits it; `start()` retries recovery on the same handle. Get, list, and `from_id` omit it. String/repr are redacted; use `as_headers()` to authenticate requests, but do not log the returned dictionary because it contains the raw token.
