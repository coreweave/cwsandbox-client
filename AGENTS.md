...
- `endpoint_share_token`: Create-only `EndpointShareToken` for `auth=SHARE_TOKEN` HTTPS URLs. Present on the create handle after Create / FromTemplate. Get, list, and `from_id` omit it. String/repr are redacted; use `as_headers()` to authenticate requests.
