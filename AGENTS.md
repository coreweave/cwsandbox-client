...
- `Sandbox.delete(sandbox_id, missing_ok=False, retry_transient_unavailable=True)`: Delete sandbox by ID, return `OperationRef[None]`. Raises `SandboxError` on failure. Set `missing_ok=True` to suppress `SandboxNotFoundError` for already-deleted sandboxes. Retries up to 3 attempts within the timeout on gRPC `UNAVAILABLE` carrying `RetryInfo` (a not-found on a retry counts as deleted); pass `retry_transient_unavailable=False` for one attempt (per call only; `SandboxDefaults`/`Session` do not apply to this classmethod). Plain `stop()` and gateway `read_file()` follow the same policy via `SandboxDefaults.retry_transient_unavailable`.
