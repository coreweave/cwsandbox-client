...
Key methods:
- `run(*args, **kwargs)`: Create and start sandbox (CreateSandbox), returning after backend acceptance and any needed share-token recovery. Accepts advanced configuration kwargs (see below). Rejects removed kwargs (`profile_ids`, `profile_names`, `s3_mount`, `ports`, `max_timeout_seconds`) with `TypeError`.
...
- `run_from_file(contents, /, *, primary_service, file_type=COMPOSE, image_overrides=None, default_resources=None, **kwargs)`: Create from Compose YAML (CreateSandboxFromFile). `contents` is a filesystem path (`str` / `Path`) or raw YAML `bytes` (sent as-is; 256 KiB cap). A `str` is always opened as a path; encode Compose text with `.encode("utf-8")`. Pull-only: leftover `build:` is `CWSANDBOX_NOT_IMPLEMENTED`. Same `request_id` with different bytes (including whitespace) is `CWSANDBOX_REQUEST_ID_CONFLICT`. Accepts `placement_mode`, `runner_ids`, `network`, tags, annotations, `object_storage_access`, `max_lifetime_seconds`, `request_id`. Rejects volumes, published services, `runtime_class`, `image_pull_credentials`, `network_ids`, `build_contexts`, and non-strict `placement_spillover`. Reuse `wait()`; do not wait for `PREPARING`. Get returns the translated spec, not the source file.
- `start()`: Send create request, return `OperationRef[None]`. Call `.result()` to block until backend accepts and any needed share-token recovery finishes (15s overall, up to 5s per replay, capped by a lower `request_timeout_seconds`). Freezes one create `request_id` for idempotent retries. If recovery exhausts, calling `start()` again replays the same request without creating another sandbox.
...
- `endpoint_share_token`: Create-only `EndpointShareToken` for `auth=SHARE_TOKEN` HTTPS URLs. Present on the create handle after Create / FromTemplate. The client replays an accepted create up to three times when the first response omits it, within a 15s budget and with a 5s per-RPC cap; `start()` retries recovery on the same handle. Get, list, and `from_id` omit it. String/repr are redacted; use `as_headers()` to authenticate requests, but do not log the returned dictionary because it contains the raw token.
- `service_endpoints`: Tuple of `HttpsEndpointStatus` (port, name, kind, auth, url, applied `request_timeout_seconds`) for HTTPS product endpoints; timeout remains after URL suppression; unspecified/unknown proto auth is omitted (the matching URL can still appear on `service_urls`)
...
- `Sandbox.delete(sandbox_id, missing_ok=False)`: Delete sandbox by ID, return `OperationRef[None]`. Raises `SandboxError` on failure. Set `missing_ok=True` to suppress `SandboxNotFoundError` for already-deleted sandboxes. Retries up to 3 attempts within the timeout on gRPC `UNAVAILABLE` carrying a `RetryInfo` delay of at most 10 s (a not-found on a retry counts as deleted). Plain `stop()` and gateway `read_file()` follow the same policy; exec, streams, creates, and writes do not.
...
**`Service` / `ServiceVisibility` / `ServiceProtocol` / `Endpoint` / `EndpointShareToken` / `HttpsEndpointStatus` / `TlsPassthroughEndpointStatus`** (`_types.py`): Typed service ports replace beta string ingress/egress modes. Pass as `services=` on `run()` / defaults. HTTPS is create-time only: `endpoint=Endpoint(kind=HTTPS, auth=OPEN|SHARE_TOKEN)` on PUBLIC. `SHARE_TOKEN` returns a create-only `EndpointShareToken` at `Sandbox.endpoint_share_token` (Create / FromTemplate); Get, list, and `from_id` omit it. String/repr are redacted. Use `as_headers()` for the preferred header authentication, but do not log the returned dictionary because it contains the raw token; `get_secret_value()` is an explicit escape hatch for the backend's query-parameter fallback and must not be logged either. If the first create response omits the token while HTTPS URL confirmation converges, the client replays the same request up to three times; `start()` retries recovery on the same handle without creating another sandbox. Optional `Endpoint.request_timeout_seconds` is the server-side HTTPS request clock (504 while the sandbox stays alive). The SDK only checks that the value is an `int`. It is not `Sandbox.run(request_timeout_seconds=...)` (client RPC deadline). Omit/`0` is the platform default (15s on serverless). The server accepts `0` or `[15, 900]`. On create-from-template, `0` is replace-on-presence and does not clear a template timeout back to the platform default. Applied timeout is echoed on `Sandbox.service_endpoints`. TLS passthrough is create-time only: `endpoint=Endpoint(kind=TLS_PASSTHROUGH)` on PUBLIC, auth and timeout omitted (`None`, not `0`). Create, Get, list, and `from_id` fill `Sandbox.service_addresses` as `TlsPassthroughEndpointStatus`; use `address` host as TLS SNI. Mixed HTTPS+TLS on one sandbox is supported. Listen-only PUBLIC or PRIVATE plus a product endpoint is `CWSANDBOX_NOT_IMPLEMENTED`.
...
- `disable_signal_handlers()`: Process-wide, sticky signal opt-out for embedded hosts
- `_reset_for_testing()`: Restores handlers and clears registration plus disable state
...
These were never public 0.26 SDK surfaces and remain unsupported until v1 backends implement them: Settings **WIF admin**, **SecretStore admin** CRUD, **NetworkService** / `network_ids`, **TOKEN** product endpoints, mixed PRIVATE+PUBLIC on one sandbox. Create-time `Secret` inject and FSS bucket get/set still work.
...
**Lazy-Start Model**: `Sandbox.run()` returns after the backend accepts the request and any needed share-token recovery finishes; it does NOT wait for RUNNING status. Recovery can add up to 15s when an accepted create omits the token. Blocking for readiness happens explicitly via `.wait()`.
